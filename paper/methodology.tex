...
The agent is built on the ReAct paradigm, in which a model alternates between
...
\begin{description}
    ...
    narrative text using scientific embeddings such as SciBERT. Domain-specific
    embeddings are used rather than general-purpose ones because scientific
    vocabulary is precisely where general embeddings are weakest.

    \item[\texttt{TableRetriever}.] A structured parser that converts tables into a
    normalised form \emph{while preserving the two-dimensional coordinates of every
    cell}. Coordinate preservation is not incidental detail. The bounding boxes
    ...
    visual patches using vision-language multi-vector retrieval, locating charts,
...
This linkage is performed by a cross-attention layer, in which the text
representation queries the visual representation:
\begin{equation}
    \mathbf{Z}_{\text{fused}} = \text{Softmax}\left(\frac{\mathbf{Q}_t \mathbf{K}_v^{\top}}{\sqrt{d_k}}\right) \mathbf{V}_v + \mathbf{H}_t
\end{equation}

\noindent Read plainly, this says: \emph{for each text token, measure how strongly
it matches each visual patch; convert those match scores into weights; use the
weights to build a summary of the visual evidence most relevant to that token; and
add this summary to the token's own representation.} The three factors play the
following roles:

\begin{itemize}
    \item $\mathbf{Q}_t \mathbf{K}_v^{\top}$ scores every text token against every
    visual patch --- the raw compatibility between what the text says and what the
    page shows.
    \item Division by $\sqrt{d_k}$ keeps these scores in a numerically stable range
    before the softmax, which is standard practice in scaled dot-product attention.
    \item The final $+\,\mathbf{H}_t$ is a residual connection: the visual evidence
    \emph{augments} the text representation rather than replacing it, so textual
    meaning is not lost when visual grounding is added.
\end{itemize}
...
\begin{enumerate}
    ...
    from?''} Integrated gradients identify the source sentences that most influenced
...
The framework will be evaluated on \textbf{SPIQA}, a dataset of over 270{,}000
...
\begin{itemize}
    \item \textbf{Single-Pass Multimodal RAG} (e.g.\ LLaVA-RAG) --- multimodal but
    ...
    \item \textbf{Text-Only Agentic Reasoning} (e.g.\ ReAct) --- iterative but not
