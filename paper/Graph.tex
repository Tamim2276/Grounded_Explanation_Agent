...
can be built node-for-node in a framework such as LangGraph, with nothing left to
...
\node[ret] (n4) at (-4.6,-2.3) {\textbf{N4} Dual Index Build\\\scriptsize ColPali + text FAISS};
...
\node[ret] (n6b) at (0,-4.9)   {\textbf{N6b} TableRetriever\\\scriptsize Cell-coord parser};
...
\node[fus] (n8) at (0,-9.0)    {\textbf{N8} Cross-Modal Fusion\\\scriptsize Evidence buffer $\mathcal{B}_t$};
...
tools that hold exclusive authority to introduce evidence; the teal node maintains
...
Two search indexes are built from the same set of pages: one holds visual
embeddings of each page image (for finding tables and figures), the other holds
text embeddings of each chunk (for finding claims). Because both indexes point back
...
Extracts a table's structure while keeping track of exactly where each cell sits on
the page. This matters because the bounding boxes generated later (N10b) are only
...
\paragraph{N8 --- Cross-Modal Fusion.}
...
step. This buffer is the only thing the generator will ever see; nothing after this
...
Writes the answer using only what is in the evidence buffer. It cannot search on
...
Highlights which retrieved sentences most influenced each part of the answer.
